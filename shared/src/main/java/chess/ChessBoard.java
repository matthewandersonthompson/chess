...
public class ChessBoard {
    //Step 1: We add a chess board. It's private, holds a ChessPiece, in an array of an array, which is the square, = a new ChessPiece in an 8,8
    ...
    public void addPiece(ChessPosition position, ChessPiece piece) {
        //Step 2: Add a piece. Select this square, and from the position getRow and getColumn, but remember to -1 since this is a 0 based index. All that = the piece
        this.square[position.getRow() - 1][position.getColumn() - 1] = piece;
    }

    public ChessPiece getPiece(ChessPosition position) {
        //Step 3: We press enter on the getPiece step
        //Step 4: Find out what piece is at a specific location. We want to return it as well. So select this square, from a position, and getRow and getColumn -1
        return this.square[position.getRow() - 1][position.getColumn() - 1];
    }

    public void resetBoard() {
        //Step 5: We press enter on the resetBoard step
        //First, we will clear the board
        //Step 6: start a for loop, with int row equal to 0, semi, as long as row is less than 8, semi, then incrementing through row with a double plus. But it isn't over so open a curly
        for (int row = 0; row < 8; row++) {
            //Step 7: start a nested for loop, with int column equal to 0, semi, as long as column is less than 8, semi, then incrementing through column with a double plus. But it isn't over so open a curly
            for (int column = 0; column < 8; column++) {
                //Step 8: then take this square at the row and column, and make it equal to null, semi
                this.square[row][column] = null;
            }
        }
        //Step 9: the Pawns
        // White Pawns
